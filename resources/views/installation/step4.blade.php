...
@section('content')
    ...
    <div class="pb-2">
        <div class="progress cursor-pointer" role="progressbar" aria-label="Demandium Software Installation"
            ...
            <div class="progress-bar width-p-80"></div>
        </div>
    </div>
        ...
        <div class="p-4 mb-md-3 mx-xl-4 px-md-5">
            ...
            @if(session()->has('error'))
                ...
                <div class="text-center">
                    <a href="{{ route('force-import-sql') }}" class="btn btn-danger px-sm-5 showLoder">
                        {{translate('Force Import Database')}}</a>
                </div>
            @else
                <div class="text-center">
                    <a href="{{ route('import_sql',['token'=>bcrypt('step_5')]) }}"
                       class="btn btn-dark px-sm-5 showLoder">{{translate('Click Here')}}</a>
                </div>
            @endif
