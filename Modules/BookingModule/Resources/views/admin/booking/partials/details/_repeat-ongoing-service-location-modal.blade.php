<div class="modal fade" id="repeatServiceLocationModal--{{$booking['id']}}" tabindex="-1"
     aria-labelledby="repeatServiceLocationModalLabel"
     aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <form action="{{route('admin.booking.repeat.change-service-location', [$booking['id']])}}" method="POST">
            @csrf
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="bg-light p-3 rounded">
                        ...
                        <div class="customer-details mt-3">
                            ...
                            <input type="hidden" name="contact_person_number" value="{{ $booking?->booking?->service_address->contact_person_number ?? '' }}">
                            <input type="hidden" name="address_label" value="{{ $booking?->booking?->service_address->label ?? '' }}">
                            <input type="hidden" name="address" value="{{ $booking?->booking?->service_address->address ?? '' }}">
                            <input type="hidden" name="latitude" value="{{ $booking?->booking?->service_address->latitude ?? '' }}">
                            <input type="hidden" name="longitude" value="{{ $booking?->booking?->service_address->longitude ?? '' }}">
                            <input type="hidden" name="city" value="{{ $booking?->booking?->service_address->city ?? '' }}">
                            <input type="hidden" name="street" value="{{ $booking?->booking?->service_address->street ?? '' }}">
                            <input type="hidden" name="zip_code" value="{{ $booking?->booking?->service_address->zip_code ?? '' }}">
                            <input type="hidden" name="country" value="{{ $booking?->booking?->service_address->country ?? '' }}">
                                ...
                                @endif
                                    <div class="media-body d-flex justify-content-between">
                                        <div>
                                            <h5 class="c1 mb-3">
                                                @if (!$booking?->is_guest && $booking?->customer)
                                                    <span class="c1 updated_customer_name">{{ Str::limit($booking?->service_address->contact_person_name, 30) }}</span>
                                                @else
                                                    <span class="updated_customer_name">{{ Str::limit($booking?->service_address->contact_person_name ?? '', 30) }}</span>
                                                @endif
                                            </h5>
                                            <ul class="list-info">
                                                @if ($phone)
                                                    <li>
                                                        ...
                                                        <a href="tel:{{ $booking?->service_address->contact_person_number }}" id="updated_customer_phone">{{ $booking?->service_address->contact_person_number }}</a>
                                                    </li>
                                                @endif
                                                    <li>
