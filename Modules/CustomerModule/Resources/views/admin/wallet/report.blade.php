                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form
                                action="{{route('admin.customer.wallet.report', ['transaction_type'=>$queryParams['transaction_type']])}}"
                                method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-lg-4 col-sm-6 mb-30">
                                        <select class="js-select customer__select" name="customer_ids[]"
                                                id="user_selector__select" multiple>
                                            <option value="all">{{translate('Select All')}}</option>
                                            @foreach($customers as $customer)
                                                <option
                                                    value="{{$customer['id']}}" {{array_key_exists('customer_ids', $queryParams) && in_array($customer['id'], $queryParams['customer_ids']) ? 'selected' : '' }}>{{$customer['first_name']. ' ' .$customer['last_name']}}
                                                    ({{$customer['phone']}})
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-lg-4 col-sm-6 mb-30">
                                        <select class="js-select" name="trx_type">
                                            ...
                                            <option
                                                value="all"{{array_key_exists('trx_type', $queryParams) && $queryParams['trx_type']=='all'?'selected':''}}>{{translate('All')}}</option>
                                            @foreach(WALLET_TRX_TYPE as $type)
                                                <option
                                                    value="{{$type}}" {{array_key_exists('trx_type', $queryParams) && $queryParams['trx_type']==$type?'selected':''}}>{{translate($type)}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-4 col-sm-6 mb-30">
