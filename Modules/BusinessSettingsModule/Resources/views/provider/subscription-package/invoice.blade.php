                ...
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex gap-4">
                        <div>
                            ...
                            <div class="fs-10">{{ request()->user()->provider->company_name }}</div>
                        </div>
                        <div>
                            <div>{{translate('Phone')}}</div>
                            <div class="fs-10">{{ request()->user()->provider->company_phone }}</div>
                        </div>
                        <div>
                            <div>{{translate('Email')}}</div>
                            <div class="fs-10">{{ request()->user()->provider->company_email }}</div>
                        </div>
                    </div>
                    <div class="d-flex flex-column gap-1 align-items-end text-right">
                        <div>{{translate('Invoice of')}} ({{currency_code()}})</div>
                        <h5 class="text-primary fw-700 mb-0 lh-1 mt-1">{{with_currency_symbol($transaction->credit + $transaction?->packageLog?->vat_amount )}}</h5>
                    </div>
                </div>
            </div>
            <div class="invoice-card__body">
                @if($transaction->trx_type != 'subscription_refund')
                    <div class="meta-info d-flex mb-30">
                        ...
                        <div>
                            @php
                                $start = \Carbon\Carbon::parse($transaction?->packageLog?->start_date)->startOfDay()->subDay();
                                $end = \Carbon\Carbon::parse($transaction?->packageLog?->end_date)->endOfDay();
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <tr>
                            ...
                            @else
                                <td>{{ translate('refunded')}}</td>
                            @endif
                            <td>{{ \Carbon\Carbon::parse($transaction?->created_at)->format('M d, Y') }}</td>

                            @if($transaction->trx_type != 'subscription_refund')
                                <td>{{ with_currency_symbol($transaction?->packageLog?->vat_amount) }}</td>
                                <td>{{ with_currency_symbol($transaction?->packageLog?->package_price) }}</td>
                                <td>{{ with_currency_symbol($transaction?->packageLog?->package_price + $transaction?->packageLog?->vat_amount) }}</td>
                            @else
