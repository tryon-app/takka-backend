...
<head>
    ...
    <title>{{translate('Account Suspend!')}}</title>
...
<div class="box">
    <div class="top-wrap-box">
        <img src="{{asset('public/assets/admin-module/img/email/renewal-reminder.png')}}" alt="{{translate('Logo')}}"
             width="140"/>

        <h3 style="margin-top: 20px">{{translate('Account Suspend')}} !</h3>
        <h5>{{translate('Hi')}} {{$provider?->company_name}},</h5>
        <p>{{translate('We are writing to inform you that your subscription to') }} {{(business_config('business_name', 'business_information'))->live_values }} {{ translate(', has ended on') }} {{ $provider->packageSubscriptions->package_end_date }} {{ translate('. Your account has been placed on hold & services have been suspended accordingly.')  }}
        </p><br><br>
        <p>{{ translate('To renew your subscription & maintain uninterrupted service, please follow the link below') }}:</p><br><br>
        <a href="{{ route('provider.subscription-package.details')}}">{{ app('url') }}/subscription-package/details</a>
    </div>
